...
import com.quan.core.constant.exception.FailToCastObjectException;
import com.quan.core.constant.exception.FailToGetValueException;
import com.quan.core.constant.exception.FailToSetValueException;
...
public class Mirror<T> {
    ...
    public static <T> Mirror<T> me(Type type) {
        ...
        if (mir == null) {
            mir = (Mirror<T>) Mirror.me(Langs.getTypeClass(type));
            mir.type = type;
     ...
     *
     * @param method       方法对象
     * @param errmsgFormat 如果本方法即不是 Getter 也不是 Setter 的回调, 则根据这个消息模板抛出一个运行时异常。 这个字符串格式是个
    ...
    public static void evalGetterSetter(final Method method,
        ...
        evalGetterSetter(method, callback, new Callback<Method>() {
            public void invoke(Method method) {
                throw Langs.makeThrow(errmsgFormat,
                        method.getName(),
     ...
     * 但是父类不包括 Object 类，并且，如果子类的属性如果与父类重名，将会将其覆盖
     *
     * @param noStatic 返回的列表中是否包括 static 标记的属性, true: 不包括。false:包括
     * @param noFinal  返回的列表中是否包括 final 标记的属性, true: 不包括。false:包括
     * @return 属性列表
     */
    ...
    public Type[] getGenericsTypes() {
        if (type instanceof ParameterizedType) {
            return Langs.getGenericsTypes(type);
        }
        return null;
    }
...
//                    value = Castors.me().castTo(value, field.getType());
    ...
    public String getTypeId() {
        if (null == _type_id) {
            if (null != type && type instanceof ParameterizedType) {
                ...
                _type_id = String.format("%s<%s>", klass.getName(), Langs.concat(",", list));
            }
            // TODO 这里应该作一些更多的判断
            else {
    ...
    public Class<?> getWrapperClass() {
        if (!klass.isPrimitive()) {
            ...
            throw Langs.makeThrow("Class '%s' should be a primitive class", klass.getName());
        }
        // TODO 用散列能快一点
        if (is(int.class))
        ...
        throw Langs.makeThrow("Class [%s] has no wrapper class!", klass.getName());
    }
...
//                        continue OUT;
                ...
                try {
                    for (int i = 0; i < ss.length; i++) {
                        ...
                        if (className.length() > 0 && className.charAt(0) == '?')
                            ...
                            if (pos < 0)
                                re[i] = Langs.loadClass(className);
                            else
                                re[i] = Langs.loadClass(className.substring(0, pos));
                        }
                    }
                    return re;
                } catch (ClassNotFoundException e) {
    ...
    public static <T> Class<T> getTypeParam(Class<?> klass, int index) {
        ...
        if (index >= 0 && index < types.length) {
            Type t = types[index];
            Class<T> clazz = (Class<T>) Langs.getTypeClass(t);
            if (clazz == null)
                throw Langs.makeThrow("Type '%s' is not a Class", t.toString());
            return clazz;
        }
        throw Langs.makeThrow("Class type param out of range %d/%d", index, types.length);
    }
