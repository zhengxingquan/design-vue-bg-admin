...
public  class Disks {
    ...
    public static String getRelativePath(String base, String path, String equalPath) {
        ...
        sb.append(Langs.concat(pos, ff.length - pos, '/', ff));
        if (path.endsWith("/"))
            sb.append('/');
        return sb.toString();
    }
    ...
    public static String getIntersectPath(String ph0, String ph1, String dft) {
        ...
        // 得到
        String re = Langs.concat(0, pos, "/", ss0).toString();
    ...
    public static String getCanonicalPath(String path) {
        ...
        StringBuilder sb = Langs.concat("/", paths);
        if (path.startsWith("/"))
            sb.insert(0, '/');
        if (path.endsWith("/"))
            sb.append('/');
    ...
    public static String appendPath(String... phs) {
        String[] paths = Langs.without(phs, null);
        if (null != paths && paths.length > 0) {
            // zozoh: 嗯下面的逻辑木有必要了吧
            // if (null == paths[0])
            // paths[0] = "/";
            String str = Langs.concat("/", paths).toString();
            String[] ss = Strings.splitIgnoreBlank(str, "/");
            str = Langs.concat("/", ss).toString();
