...
import com.quan.common.exception.OtherException;
import com.quan.common.meta.Email;
...
public class Strings {
    ...
    private static Map<String, Object> __html_replace_map = Langs.map("'&[A-Z]?[a-z0-9]{2,};':null,'&':'&amp;','<':'&lt;'");
    private static Map<String, Object> __html_replace_map_full = Langs.map("'&[A-Z]?[a-z0-9]{2,};':null,'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'");
    ...
    public static <T> String join2(String sp, T[] array) {
        return Langs.concat(sp, array).toString();
    }
    ...
    public static <T> String join(int off, int len, String sp, T[] array) {
        return Langs.concat(off, len, sp, array).toString();
    }
    ...
    public static <T> String join(String sp, Collection<T> coll) {
        return Langs.concat(sp, coll).toString();
    }
            ...
            if (c == '\\') {
                ...
                switch (c) {
                    ...
                    case '\\':
                        ...
                        break;
                    default:
                        throw new OtherException("evalEscape invalid char[%d] '%c'  : %s", i, c, str);
                }
            }
            // 否则添加
            else {
    ...
    public static String cutStr(int length, String s, String supply) {
        ...
        if (Langs.isEmpty(length) || Langs.isEmpty(s))
            return null;
